...
pub fn render(f: &mut Frame, app: &App, area: Rect) {
    ...
    let items: Vec<ListItem> = if app.tag_view.tags.is_empty() {
        vec![ListItem::new(Span::styled(
            "  no tags — use `torii tag create` to create one",
        ...
        app.tag_view.tags.iter().enumerate().map(|(i, t)| {
            ...
            let line = Line::from(vec![
                ...
                Span::styled(format!("{:<40}", &t.message), Style::default().fg(C_WHITE)),
    ...
    // Status block
    let status_text = app.tag_view.status.as_deref().unwrap_or("ready");
    let status_color = if app.tag_view.status.is_some() { C_GREEN } else { C_DIM };
